...
下一步不再继续纯理论学习，应该直接做一个小项目，在实践里补齐 harness / tools / workflow / memory 的理解。
...
3. DeepSeek 负责判断和回答
4. tools 负责读写笔记
5. memory 先用简单文本搜索，不急着上向量库
```

建议工具：

```text
search_notes(query)   搜索已有笔记
read_note(path)       读取某篇笔记
save_note(title,text) 保存新的学习总结
finish(answer)        结束本轮任务
```
...
User input
  -> agent node
  -> tools node
       search_notes / read_note / save_note / finish
  -> 如果 finish，结束
  -> 否则回 agent node
```
...
```text
notes/ 里存 markdown
search_notes 先用关键词或简单字符串匹配
流程跑通后，再替换成 embedding top-k
最后再加 rerank
```
...
先做能跑的 agent
再做更好的 memory
最后再接 embedding / rerank / Dify 对照
```

主线仍然是：

```text
流程设计能力 > 状态建模能力 > 工具边界设计 > Python 语法细节
```
